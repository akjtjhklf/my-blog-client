...
export const FriendProvider = ({ children }) => {
    ...
    const handleSendFriendRequest = (friend) => {
        axios
        .put(
        `https://meowblog.onrender.com/users/${user._id}/send-friend-request`,
        { friend },
    ...
    const handleAcceptFriendRequest = (friendRequest, acceptRequest) => {
        ...
        axios.put(
        `https://meowblog.onrender.com/users/${user._id}/accept-friend-request`,
    ...
    const handleRemoveFriend = (friendId) => {
        // Chấp nhận hoặc từ chối lời mời kết bạn
        axios.put(
        `http://meowblog.onrender.com/users/${user._id}/remove-friend`,
        { friendId},
        { headers: { Authorization: `Bearer ${token}` } }
        )
        .then((response) => {
