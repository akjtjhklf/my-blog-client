...
function PostDetail() {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        const postResponse = await axios.get(`https://meowblog.onrender.com/posts/${postId}`);
        setPost(postResponse.data.data);
        const isAuthor = user._id === postResponse.data.data.author.userdata._id;
  ...
  const handleOk = async () => {
    ...
    const updatedPostData = await axios.get(`https://meowblog.onrender.com/posts/${postId}`);
